...
#define NEMESIS_SCREAM "zombie_plague/nemesis2.wav"
...
new const ZP_TEAM_NAMES[][] = {"ZOMBIE , HUMAN", "ZOMBIE", "HUMAN", "ZOMBIE , HUMAN", "NEMESIS",
			...
			"ZOMBIE , HUMAN , NEMESIS , SURVIVOR", "SNIPER", "ZOMBIE , SNIPER", 
                           "HUMAN , SNIPER", "HUMAN , SNIPER , SURVIVOR", "ZOMBIE , HUMAN , SNIPER", "NEMESIS , SNIPER",
                           "ZOMBIE , NEMESIS, SNIPER", "HUMAN, NEMESIS, SNIPER", "ZOMBIE , HUMAN , NEMESIS , SNIPER",
...
new g_MsgSync2, g_status_sync // message sync objects
new g_trailSpr, g_exploSpr, g_flameSpr, g_smokeSpr, g_glassSpr // grenade sprites
new g_ExploSprFrost, g_iFlareFrost, g_ExploSprFire, g_iFlareFire // Grenades Sprites added
...
new g_buycountround[33] // buy count for a player in a round
...
Array:g_zclass2_spd, Array:g_zclass2_grav, Array:g_zclass2_kb, Array:g_zclass2_lvl, Array:g_zclass_new
...
Array:sound_win_humans_ismp3, Array:sound_win_no_one_ismp3, Array:zombie_infect, Array:human_death,
...
Array:sound_ambience6_ismp3, m_spriteTexture
...
cvar_fragsinfect, cvar_fragskill, cvar_fragskillnemesis, cvar_humanarmor, cvar_zombiesilent, cvar_removedropped,
...
cvar_buyzonetime, cvar_huddisplay, cvar_maxbuyround,
...
Float:g_cached_leapsurvivorcooldown, g_cached_leapsniper, Float:g_cached_leapsnipercooldown, Float:g_cached_buytime
...
public plugin_precache()
{
	...
	g_ExploSprFrost = precache_model("sprites/ze_wwdp_grenade/buz_kurukafa.spr")
	g_iFlareFrost = precache_model("sprites/ze_wwdp_grenade/buz_parcacigi.spr")

	g_ExploSprFire = precache_model("sprites/ze_wwdp_grenade/ates_kurukafa.spr")
	g_iFlareFire = precache_model("sprites/ze_wwdp_grenade/ates_parcacigi.spr")
	...
	for (i = 0; i < ArraySize(human_death); i++)
	{
		ArrayGetString(human_death, i, buffer, charsmax(buffer))
		engfunc(EngFunc_PrecacheSound, buffer)
	}
	...
	engfunc(EngFunc_PrecacheSound, NEMESIS_SCREAM)
}
...
public plugin_init()
{
	...
	if (g_ambience_sounds[AMBIENCE_SOUNDS_INFECTION] || g_ambience_sounds[AMBIENCE_SOUNDS_NEMESIS] || g_ambience_sounds[AMBIENCE_SOUNDS_SURVIVOR] || g_ambience_sounds[AMBIENCE_SOUNDS_SWARM] || g_ambience_sounds[AMBIENCE_SOUNDS_PLAGUE] || g_ambience_sounds[AMBIENCE_SOUNDS_SNIPER])
	...
	cvar_maxbuyround = register_cvar("zp_maxbuyround", "3")
	...
	cvar_fragskillnemesis = register_cvar("zp_human_frags_for_kill_nemesis", "1")
	...
	cvar_sniperdamage = register_cvar("zp_sniper_damage", "6000")
	...
	register_cvar("zp_version", "Ultimate", FCVAR_SERVER|FCVAR_SPONLY)
	set_cvar_string("zp_version", "Ultimate")
	...
	// Format mod name
	formatex(g_modname, charsmax(g_modname), "Zombie Escape")
...
public logevent_round_end()
{
	...
	if ((g_ambience_sounds[AMBIENCE_SOUNDS_NEMESIS] && g_nemround) || (g_ambience_sounds[AMBIENCE_SOUNDS_SURVIVOR] && g_survround) || (g_ambience_sounds[AMBIENCE_SOUNDS_SNIPER] && g_sniperround) || (g_ambience_sounds[AMBIENCE_SOUNDS_SWARM] && g_swarmround) || (g_ambience_sounds[AMBIENCE_SOUNDS_PLAGUE] && g_plagueround) || (g_ambience_sounds[AMBIENCE_SOUNDS_INFECTION] && !g_nemround && !g_survround && !g_swarmround && !g_plagueround && !g_sniperround))
	{
	...
	if (!fnGetZombies())
	{
		...
		ambience_sound_stop()
		// Human team wins
		set_dhudmessage(85, 170, 255, -1.0, 0.26, 0, 6.0, 3.0)
		show_dhudmessage(0, "===== HUMAN WINS! ====^n^n^nEscape Success!")
	...
	else if (!fnGetHumans())
	{
		...
		ambience_sound_stop()
		// Zombie team wins
		set_dhudmessage(255, 85, 85, -1.0, 0.26, 0, 6.0, 3.0)
		show_dhudmessage(0, "===== ZOMBIE WINS! ====^n^n^nEscape Failed!")
...
public showStatus(id)
{
	if(!is_user_bot(id) && is_user_connected(id))
	{
		...
		if (team1 == team2)
		{
			...
			ShowSyncHudMsg(id, g_status_sync, "%s - %d HP / %d Armor / %d Points", name, get_user_health(pid), get_user_armor(pid), g_ammopacks[pid])
		}
		else
		{
			set_hudmessage(color1, 50, color2, -1.0, 0.60, 1, 0.01, 3.0, 0.01, 0.01, -1)
...
public fw_PlayerKilled(victim, attacker, shouldgib)
{
	...
	if (g_zombie[victim] || g_sniper[victim])
	{
	...
	if ((g_nemesis[attacker] && get_pcvar_num(cvar_nemignorefrags)) || (g_survivor[attacker] && get_pcvar_num(cvar_survignorefrags)) || (g_sniper[attacker] && get_pcvar_num(cvar_sniperignorefrags)))
	...
	if (!g_zombie[attacker] && !g_nemesis[victim] && get_pcvar_num(cvar_fragskill) > 1)
		UpdateFrags(attacker, victim, get_pcvar_num(cvar_fragskill) - 1, 0, 0)
	
	// Human killed nemesis, add up the extra frags for kill
	if (!g_zombie[attacker] && g_nemesis[victim] &&  get_pcvar_num(cvar_fragskillnemesis) > 1)
		UpdateFrags(attacker, victim, get_pcvar_num(cvar_fragskillnemesis) - 1, 0, 0)
...
public fw_PlayerKilled_Post(victim, attacker, shouldgib)
{
	...
	if (get_pcvar_num(cvar_deathmatch))
	{
		...
		if ((g_zombie[victim] && !g_nemesis[victim] && !get_pcvar_num(cvar_respawnzomb)) || (!g_zombie[victim] && !g_survivor[victim] && !g_sniper[victim] && !get_pcvar_num(cvar_respawnhum)) || (g_nemesis[victim] && !get_pcvar_num(cvar_respawnnem)) || (g_survivor[victim] && !get_pcvar_num(cvar_respawnsurv)) || (g_sniper[victim] && !get_pcvar_num(cvar_respawnsniper)))
			return;
...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage, damage_type)
{
	...
	if (g_survround || g_nemround || g_swarmround || g_plagueround || g_sniperround || fnGetHumans() == 1)
...
public fw_PlayerPreThink(id)
{
	...
	else if (g_zombie[id] && !g_nemesis[id])
...
show_menu_game(id)
{
	...
	// Title
	len += formatex(menu[len], charsmax(menu) - len, "\yZombie Escape by \rMeliodas^n\yVersion \r1.4^n^n", g_modname)
	
	// 1. Buy weapons
	if (get_pcvar_num(cvar_buycustom))
		len += formatex(menu[len], charsmax(menu) - len, "\r[\w1.\r] \wGet Weapon^n", id, "MENU_BUY")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d[1.] Get Weapon^n", id, "MENU_BUY")
	
	// 2. Extra items
	if (get_pcvar_num(cvar_extraitems) && g_isalive[id])
		len += formatex(menu[len], charsmax(menu) - len, "\r[\w2.\r] \wExtra Items^n", id, "MENU_EXTRABUY")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d[2.] Extra Items^n", id, "MENU_EXTRABUY")
	
	// 3. Zombie class
	if (get_pcvar_num(cvar_zclasses))
		len += formatex(menu[len], charsmax(menu) - len, "\r[\w3.\r] \yZombie Class^n^n", id,"MENU_ZCLASS")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d[3.] Zombie Class^n^n", id,"MENU_ZCLASS")
	
	// 4. Human Class
		len += formatex(menu[len], charsmax(menu) - len, "\r[\w4.\r] \yHuman Class^n")
	
	// 5. Unstuck
	 if (g_isalive[id])
		len += formatex(menu[len], charsmax(menu) - len, "\r[\w5.\r] \wUnstuck^n", id, "MENU_UNSTUCK")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d[5.] UnstuckL^n", id, "MENU_UNSTUCK")
	
	// 6. Knife Menu
	         len += formatex(menu[len], charsmax(menu) - len, "\r[\w6.\r] \wKnife Menu^n^n", id, "MENU_BICAK")

         // 7. VIP Menu
         if (zv_get_user_flags(id) & ZV_MAIN)
	         len += formatex(menu[len], charsmax(menu) - len, "\r[\w7.\r] \yVIP Menu^n")
	else
	         len += formatex(menu[len], charsmax(menu) - len, "\d[7.] VIP Menu^n")
	
	// 8. Join spec
	if (userflags & g_access_flag[ACCESS_ENABLE_MOD])
		len += formatex(menu[len], charsmax(menu) - len, "\r[\w8.\r] \wSpectator^n", id, "MENU_SPECTATOR")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d[8.] Spectator^n", id, "MENU_SPECTATOR")
	
	// 9. Admin menu
	if (userflags & g_access_flag[ACCESS_ADMIN_MENU])
		len += formatex(menu[len], charsmax(menu) - len, "\r[\w9.\r] \wManager Menu", id, "MENU_ADMIN")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d[9.] Manager Menu", id, "MENU_ADMIN")
	
	// 0. Exit
	len += formatex(menu[len], charsmax(menu) - len, "^n^n\r[\w0\r].\wExit", id, "MENU_EXIT")
...
show_menu_extras(id)
{
	...
	for (item = 0; item < g_extraitem_i; item++)
	{
		...
		if ((g_zombie[id] && !g_nemesis[id] && !(team & ZP_TEAM_ZOMBIE)) || (!g_zombie[id] && !g_survivor[id] && !g_sniper[id] && !(team & ZP_TEAM_HUMAN)) || (g_nemesis[id] && !(team & ZP_TEAM_NEMESIS)) || (g_survivor[id] && !(team & ZP_TEAM_SURVIVOR)) || (g_sniper[id] && !(team & ZP_TEAM_SNIPER)))
			continue;
...
public show_menu_zclass(id)
{
	...
	for (class = 0; class < g_zclass_i; class++)
	{
		...
		ArrayGetString(g_zclass_lvl, class, buffer3, charsmax(buffer3))
		
		// Add to Classes								
		if(get_user_level(id) >= ArrayGetCell(g_zclass_lvl, class))
		{
			if (class == g_zombieclassnext[id])
			{
				if(ArrayGetCell(g_zclass_lvl, class) > 0)
				{
					formatex(menu, charsmax(menu), "\d%s %s (Level: %d)", buffer, buffer2, buffer3)
				}
				else
				{
			...
			else
			{
				if(ArrayGetCell(g_zclass_lvl, class) > 0)
				{
					formatex(menu, charsmax(menu), "%s \y%s \d(Level: %d)", buffer, buffer2, buffer3)
				}
				else
				{
		...
		{
			formatex(menu, charsmax(menu), "\d%s %s \r(Level: %d)", buffer, buffer2, buffer3)
		}
...
show_menu_admin(id)
{
	...
	if ((userflags & g_access_flag[ACCESS_MODE_MULTI]) && allowed_multi())
		len += formatex(menu[len], charsmax(menu) - len, "\r7.\w %L^n", id, "MENU_ADMIN6")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d7. %L^n", id, "MENU_ADMIN6")
	
	// 7. Plague mode command
	if ((userflags & g_access_flag[ACCESS_MODE_PLAGUE]) && allowed_plague())
		len += formatex(menu[len], charsmax(menu) - len, "\r8.\w %L^n", id, "MENU_ADMIN7")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d8. %L^n", id, "MENU_ADMIN7")
	...
	switch (key)
	{
		case 0: // Buy Weapons
		{
			...
			if (get_pcvar_num(cvar_buycustom))
			{
				...
				WPN_AUTO_ON = 0
				zp_colored_print(id, "^x04[ZE]^x01 %L", id, "BUY_ENABLED")
		...
		case 1: // Extra Items
		{
			...
			if (get_pcvar_num(cvar_extraitems))
			{
				...
				if (g_isalive[id])
				{
					if (g_buycountround[id] < get_pcvar_num(cvar_maxbuyround))
						show_menu_extras(id)
					else
						zp_colored_print(id, "^x04[ZE]^x01 Maximum purchase limit reached.")
				}
				else
					zp_colored_print(id, "^x04[ZE]^x01 %L", id, "CMD_NOT")
			}
		...
		case 2: // Zombie Classes
		{
			...
			if (get_pcvar_num(cvar_zclasses))
				...
				zp_colored_print(id, "^x04[ZE]^x01 %L", id, "CMD_NOT_ZCLASSES")
		}
		case 3: // Human Class
		{
			client_cmd(id,"say /hc")
			return PLUGIN_HANDLED;
		}
		case 4: // Unstuck
		{
			...
			if (g_isalive[id])
			{
				if (is_player_stuck(id))
				{
					...
					zp_colored_print(id, "^x04[ZE]^x01 %L", id, "CMD_NOT_STUCK")
			}
			else
				zp_colored_print(id, "^x04[ZE]^x01 %L", id, "CMD_NOT")
		}
...
buy_extra_item(id, itemid, ignorecost = 0)
{
	...
	if ((itemid == EXTRA_ANTIDOTE && (g_endround || g_swarmround || g_nemround || g_survround || g_plagueround || g_sniperround || fnGetZombies() <= 1 || (get_pcvar_num(cvar_deathmatch) && !get_pcvar_num(cvar_respawnafterlast) && fnGetHumans() == 1)))
	|| (itemid == EXTRA_MADNESS && g_nodamage[id]) || (itemid == EXTRA_INFBOMB && (g_endround || g_swarmround || g_nemround || g_survround || g_plagueround || g_sniperround)))
	{
...
public menu_zclass(id, menuid, item)
{
	...
	if(get_user_level(id) >= ArrayGetCell(g_zclass_lvl, classid))
	{
		...
		{
			zp_colored_print(id, "^x04[ZE]^x01 +10 Zombie Speed!")
		}
		else if(get_user_level(id) >= 20 && get_user_level(id) <= 29)
		{
			zp_colored_print(id, "^x04[ZE]^x01 +20 Zombie Speed!")
		}
		else if(get_user_level(id) >= 30)
		{
			zp_colored_print(id, "^x04[ZE]^x01 +30 Zombie Speed!")
		}
	}
	else
	{
...
make_a_zombie(mode, id)
{
	...
	{
		get_mapname(suankimap, 63)
		if((contain(suankimap, "de_") != -1) || (contain(suankimap, "cs_") != -1) || (contain(suankimap, "big_") != -1))
		{
			// Multi Infection Mode
			g_lastmode = MODE_MULTI
			g_nemround = true
			PlaySound(NEMESIS_SCREAM)
			
			new alive_count = fnGetAlive()
			
			// iMaxZombies is rounded up, in case there aren't enough players
			iMaxZombies = floatround(alive_count*get_pcvar_float(cvar_multiratio), floatround_ceil)
			iZombies = 0
			...
			// Round start forward
			ExecuteForward(g_fwRoundStart, g_fwDummyResult, MODE_NEMESIS, 0);
		}
		else
		{
			// Multi Infection Mode
...
zombieme(id, infector, nemesis, silentmode, rewards)
{
	...
	g_zombie_spd[id] = (float(ArrayGetCell(g_zclass_spd, g_zombieclass[id])) + extrahiz)
	...
	if (!silentmode)
	{
		...
		else
		{
			...
			ArrayGetString(human_death, random_num(0, ArraySize(human_death) - 1), sound, charsmax(sound))
			emit_sound(id, CHAN_VOICE, sound, 1.0, ATTN_NORM, 0, PITCH_NORM)
		}
	}
	else
			...
			case SECTION_SOUNDS:
			{
				...
				else if (equal(key, "HUMAN DEATH"))
				{
					...
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						// Add to sounds array
						ArrayPushString(human_death, key)
					}
				}
...
save_customization()
{
	...
	for (i = 0; i < size; i++)
	{
		if (ArrayGetCell(g_zclass_new, i))
		{
			...
			// Add level
			formatex(buffer, charsmax(buffer), "^nLEVEL = %d^n", ArrayGetCell(g_zclass_lvl, i))
			fputs(file, buffer)
		}
	}
	fclose(file)
...
public respawn_player_task(taskid)
{
	...
	if ((!g_survround || get_pcvar_num(cvar_allowrespawnsurv)) && (!g_swarmround || get_pcvar_num(cvar_allowrespawnswarm)) && (!g_nemround || get_pcvar_num(cvar_allowrespawnnem)) && (!g_plagueround || get_pcvar_num(cvar_allowrespawnplague)) && (!g_sniperround || get_pcvar_num(cvar_allowrespawnsniper)))
	{
		// Infection rounds = none of the above
		if (!get_pcvar_num(cvar_allowrespawninfection) && !g_survround && !g_nemround && !g_swarmround && !g_plagueround && !g_sniperround)
			return;
			...
			if (g_zombie[id])
			{
				...
				else // Zombies
				{
					...
					else
					{
						...
						format(clawmodel, charsmax(clawmodel), "models/dp_escape/%s", clawmodel)
...
public ShowHUD(taskid)
{
	...
	if (id != ID_SHOWHUD)
	{
		...
		ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync2, "%L %s^nHP: %d - Armor: %d - %L %s - %L %d^nLevel: %i | XP: %i / %i", ID_SHOWHUD, "SPECTATING", g_playername[id], pev(id, pev_health), get_user_armor(id), ID_SHOWHUD, "CLASS_CLASS", class, ID_SHOWHUD, "AMMO_PACKS1", g_ammopacks[id], get_user_level(id), get_user_xp(id), get_user_max_level(id))
	}
	else
	{
		// Show health, class and ammo packs
		set_hudmessage(red, green, blue, HUD_STATS_X, HUD_STATS_Y, 0, 6.0, 1.1, 0.0, 0.0, -1)
		ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync2, "%L: %d - Armor: %d - %L %s - %L %d^nLevel: %i / 70 - XP: %i / %i^nNick: %s", id, "ZOMBIE_ATTRIB1", pev(ID_SHOWHUD, pev_health), get_user_armor(ID_SHOWHUD), ID_SHOWHUD, "CLASS_CLASS", class, ID_SHOWHUD, "AMMO_PACKS1", g_ammopacks[ID_SHOWHUD], get_user_level(ID_SHOWHUD), get_user_xp(ID_SHOWHUD), get_user_max_level(ID_SHOWHUD), g_playername[id])
	}
}
...
public native_register_zombie_class(const name[], const info[], const model[], const clawmodel[], hp, speed, Float:gravity, Float:knockback, level)
{
	...
	for (i = 0; i < size; i++)
	{
		...
		// Precache clawmodel
		formatex(prec_mdl, charsmax(prec_mdl), "models/dp_escape/%s", buffer)
	...
	if (ArrayGetCell(g_zclass_new, g_zclass_i))
	{
		...
		formatex(prec_mdl, charsmax(prec_mdl), "models/dp_escape/%s", clawmodel)
		engfunc(EngFunc_PrecacheModel, prec_mdl)
	}
...
create_blast2(const Float:originF[3])
{
	...
	engfunc(EngFunc_WriteCoord, originF[0]) // x
	engfunc(EngFunc_WriteCoord, originF[1]) // y
	engfunc(EngFunc_WriteCoord, originF[2]+40) // z
	engfunc(EngFunc_WriteCoord, originF[0]) // x axis
	engfunc(EngFunc_WriteCoord, originF[1]) // y axis
	engfunc(EngFunc_WriteCoord, originF[2]) // z axis
	write_short(g_iFlareFire)
	write_byte(25)
	write_byte(25)
	write_byte(3)
	write_byte(50)
	...
	engfunc(EngFunc_WriteCoord, originF[2]+100.0)
	write_short(g_ExploSprFire)
	write_byte(10) // 30
	write_byte(255)
	message_end()
}
